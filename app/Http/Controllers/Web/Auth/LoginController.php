        ...
        public function forgotPasswordSubmit(ForgotPasswordRequest $request)
        {
            if($request->strEmail)
            {
                ...
                {              
                    ...
                    if(!$strEmail)
                    {
                        return redirect('/web/forgotPassword')->with('forgotPassword_error','User does not exist');
                    }                    
                }
                ...
                else
                {
                    ...
                    $data['url'] = url('/web/reset-password/'.base64_encode(base64_encode($intId))); 

                    try {
                         $result = Mail::to($strEmail)->send(new ForgotPasswordMail($data,'web')); 
                    ...
                    {
                        return redirect('/web/forgotPassword')->with('forgotPassword_error','Error while sending mail');
                    }
                    
                    return redirect('/web')->with('login_success','Please check your email for updating password');
                }            
            }
            else 
            {
                  return redirect('/web/forgotPassword')->with('forgotPassword_error','Enter email address');
            }
        }
    ...
    ------------------------------------------*/
        ...
        public function resetPassword($intToken)
        {
            ...
            if(!empty($arrData))
            {
                ...
                return view('web.auth.reset-password',$arrUserData);

            }
            else
            {
                return redirect('/web/forgot-password')->with('forgotPassword_error','Invalid Token');
            }
        }
        ...
        public function resetPasswordSubmit(ResetPasswordRequest $request)
        {
            ...
            if($arrChkData)
            {
                ...
                return redirect('/web/login')->with('login_sucess','Password updated successfully');
                
            }
            else
            {
                return redirect('/web/login')->with('login_error','Error while updating password');
            }
        }
