...
{
  var action = ADMINURL + '/records/getRecords';

  const table = $('#adminListingTable').DataTable(
    {
      "responsive": true,
      "processing": true,
      "bFilter": false,
  ...
  function setCustomPagingSigns() {
    ...
    wrapper.find('tbody tr').each(function () {
      ...
      $(this).find('td:nth-child(5)').addClass('theme-green semibold text-center f-18');
    })
        ...
        axios.delete(action)
          .then(function (response) {
            if (response.data.status === 'success') {
              ...
              $('#adminListingTable').DataTable().ajax.reload();
